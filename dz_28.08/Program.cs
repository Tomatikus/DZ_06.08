// На 5 нужно сделать минимум 4 задачки
...
{
    int tempsume = SumLineElements(array, i);
    if (sumLine > tempsume)
    {
        sumLine = tempsume;
        minSumLine = i;
...
BouthMatrix(firstMartrix, secondMartrix, resultMatrix);
Console.WriteLine($"Произведение первой и второй матриц: ");
PrintArray(resultMatrix);

void BouthMatrix(int[,] firstMartrix, int[,] secondMartrix, int[,] resultMatrix)
{
    ...
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        for (int j = 0; j < matrix.GetLength(1); j++)
        {
            for (int k = 0; k < matrix.GetLength(2); k++)
                matrix[i, j, z] = new Random().Next(11);
        }
...
int [,,] resultmatrix = GetArray(m,n,z);
PrintArray(resultmatrix);
*/
